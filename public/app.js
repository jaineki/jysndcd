...
function layout(content) {
  document.body.innerHTML = `<div class="app-shell">${navMarkup()}<div class="main-column">${topbarMarkup()}<main class="page fade-in">${content}</main></div>${playerMarkup()}<div id="toast" class="toast" role="status" aria-live="polite"></div><div id="login-modal" class="modal-backdrop"><div class="modal"><h2>Welcome back</h2><p>Sign in to sync likes and playlists across your devices. You can still explore the curated radio as a guest.</p><form id="login-form"><input id="login-email" type="email" placeholder="Email address" required><input id="login-password" type="password" placeholder="Password" required style="margin-top:9px"><div class="modal-actions"><button type="button" class="ghost-button" onclick="closeLogin()">Cancel</button><button class="primary-button" type="submit">Sign in</button></div></form></div></div></div>`;
  state.audio = $('#audio');
  bindAudio();
  updatePlayer();
}
...
async function renderHome() {
  layout(homeTemplate());
  renderStations('#home-stations', stations.slice(0, 7));
...
function createPlaylist() { const name = prompt('Name your playlist:', 'My OPM Mix'); if (!name || !name.trim()) return; state.playlists[name.trim()] = []; persistLocalState(); renderPlaylist(); showToast(`Created ${name.trim()}`); }
function addToPlaylist(track) { const name = prompt('Add to playlist:', 'Orange Hour'); if (!name || !name.trim()) return; const playlist = name.trim(); state.playlists[playlist] = state.playlists[playlist] || []; if (!state.playlists[playlist].some(item => item.url === track.url)) state.playlists[playlist].push(track); persistLocalState(); showToast(`Added to ${playlist}`); }
function savePlaylistStarter() { const name = 'Orange Hour: OPM'; state.playlists[name] = [...editorialTracks]; persistLocalState(); showToast('Playlist saved to your library'); }
function openUpgrade(plan = 'Popular') { window.location.href = `/premium.html?plan=${encodeURIComponent(plan)}`; }
...
async function playTrack(track, index = 0, queue = state.currentTracks) {
  ...
  try {
    ...
    for (const candidate of candidates) {
      const response = await fetch('/api/downloader/soundcloud-v2', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: candidate.url }) });
      const data = await response.json();
      if (data.success && data.result?.url) { Object.assign(track, candidate, { invalid: false }); stream = data.result.url; break; }
    }
...
function togglePlayback() { if (!state.currentTrack) { playEditorial(); return; } if (state.isPlaying) { state.audio.pause(); state.isPlaying = false; } else { state.audio.play(); state.isPlaying = true; } updatePlayer(); }
...
function toggleLike(track) { if (!track.url) { showToast('Play the song once before saving it.'); return; } const index = state.liked.findIndex(item => item.url === track.url); if (index >= 0) { state.liked.splice(index, 1); showToast('Removed from Liked Songs'); } else { state.liked.unshift(track); showToast('Added to Liked Songs'); } persistLocalState(); updatePlayer(); if (state.page === 'playlist') renderPlaylist(); }
function openQueue() { showToast(`${state.currentTracks.length || 0} songs in the current queue`); }
function openLogin() { $('#login-modal')?.classList.add('open'); }
...
async function bootAuth() {
  try {
    ...
    if (config.supabaseUrl && config.supabaseAnonKey && window.supabase) {
      ...
      if (data.session) { state.user = data.session.user; $('#user-label').textContent = state.user.email?.split('@')[0] || 'Listener'; }
    }
  } catch (_) {}
}

function bindLogin() {
  $('#login-form')?.addEventListener('submit', async event => { event.preventDefault(); if (!state.supabase) { closeLogin(); showToast('Guest mode is ready. Add Supabase keys to enable login.'); return; } const { error } = await state.supabase.auth.signInWithPassword({ email: $('#login-email').value, password: $('#login-password').value }); if (error) showToast(error.message); else { closeLogin(); showToast('Welcome back'); } });
}
